...
def encrypt(message_index, key_index):
    return message_index + key_index

def decrypt(message_index, key_index):
    return message_index - key_index
...
def open_message_and_key_file_and_crypt_contents(crypt_mode, message_location, key_location):
    with codecs.open(message_location, encoding='utf-8') as message, \
        ...
        if crypt_mode == 'd':
            crypt_and_display_all_letters(decrypt, message, key, alphabet())
        else:
            crypt_and_display_all_letters(encrypt, message, key, alphabet())
